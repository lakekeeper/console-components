<template>
  <!-- The step is controlled, not left to the stepper: clearing `compute` removes
       step 2 from the DOM, and an uncontrolled stepper keeps pointing at it —
       which is how "Start over" ended up showing step 1's title with no cards. -->
  <v-stepper-vertical v-model="currentStep">
    <template #default="{ step }">
      ...
      <v-stepper-vertical-item
        ...
        :style="embedded ? undefined : 'max-height: 60vh; overflow-y: auto'">
        <v-tabs v-model="tab">
        ...
        <!-- @vue-skip -->
        <template #next="{ next }">
          <v-btn color="primary" variant="flat" :text="finishLabel" @click="next"></v-btn>
        </template>
        <!-- @vue-skip -->
        <template #prev="{ prev }">
...
const props = defineProps<{
  ...
  // Rendered inside a pane that scrolls for it, rather than in its own dialog:
  // no inner max-height, and "Finish" only resets rather than closing anything.
  embedded?: boolean;
}>();

const emit = defineEmits<{ (e: 'finish'): void }>();

const finishLabel = computed(() => (props.embedded ? 'Start over' : 'Finish'));
...
function onClickFinish() {
  compute.value = '';
  // Back to the only step that still exists.
  currentStep.value = '1';
  emit('finish');
}
