<template>
  <div>
    ...
    <v-row dense class="mb-3">
      <v-col cols="12" md="6">
        <v-text-field
          ...
          :hint="lockLocation ? lockedHint : 'The Azure storage account holding the data'"
          persistent-hint
          :error="!profile['account-name']"
          :rules="[rules.required]"></v-text-field>
      </v-col>
      <v-col cols="12" md="6">
        <v-text-field
          density="compact"
          v-model="profile.filesystem"
          label="Filesystem *"
          placeholder="my-container"
          :readonly="lockLocation"
          :hint="lockLocation ? lockedHint : 'ADLS Gen2 filesystem — the container in blob storage'"
          persistent-hint
          :error="!profile.filesystem"
          :rules="[rules.required]"></v-text-field>
      </v-col>
    </v-row>
    ...
    <v-row dense class="mb-3">
      <v-col cols="12" md="6">
        <v-text-field
          ...
          :hint="
            lockLocation
              ? lockedHint
              : 'Folder inside the filesystem — the validation checks call this the location'
          "
      ...
      <v-col cols="12" md="6">
        <v-text-field
          ...
          hint="Optional — only for sovereign clouds or a custom endpoint"
          persistent-hint></v-text-field>
      </v-col>
    </v-row>
    ...
    <template v-else-if="authMode === 'shared-access-key'">
      <v-text-field
        density="compact"
        ...
        :type="showSecret ? 'text' : 'password'"
        :append-inner-icon="showSecret ? 'mdi-eye-outline' : 'mdi-eye-off-outline'"
        hint="Storage account access key from the Azure portal"
        persistent-hint
        :error="!credential.key"
        :rules="[rules.required]"
        @click:append-inner="showSecret = !showSecret"></v-text-field>
    </template>
      ...
      <v-expansion-panel>
        ...
        <v-expansion-panel-text>
          <v-switch
            ...
            density="compact"
            hide-details
            label="Allow abfs:// in addition to abfss://"></v-switch>
          <div class="text-caption text-medium-emphasis mt-1 mb-3">
            Accept the unencrypted scheme as well; normally left off.
          </div>
...
const authModes = computed(() => [
  ...
  { value: 'shared-access-key', title: 'Shared access key' },
